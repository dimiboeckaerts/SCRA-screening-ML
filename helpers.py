...
def NestedGroupKFoldProba(model, X, y, parameter_grid, groups, n_classes, scorer, 
                     inner_cv=GroupKFold(n_splits=4), outer_cv=GroupKFold(n_splits=4), weights=[], cpus=1):
    """
    ...
    for train_outer, test_outer in outer_cv.split(X, y, groups):
        ...
        tuned_model = GridSearchCV(model, cv=inner_cv, param_grid=parameter_grid, scoring=scorer, njobs=cpus)
        if len(weights) == 0:
...
def NestedShuffledKFoldProba(model, X, y, parameter_grid, n_classes, scorer, 
    ...
    for train_outer, test_outer in outer_cv.split(X, y):
        ...
        # define inner loop (in GridSearchCV)
        tuned_model = GridSearchCV(model, cv=inner_cv, param_grid=parameter_grid, scoring=scorer, n_jobs=cpus)
        if len(weights) == 0:
